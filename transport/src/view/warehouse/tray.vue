    ...
    <el-row class="storagebuy">
        ...
        <el-col :span="24" class="btn-form">
            <el-col :span="4" class="print">
                <el-button type="primary"  @click="newdocument" plain>新建托盘</el-button>
            </el-col>
            <el-col :span="24">
...
<script>
import myTable from '../../components/basic-info/tray-table'
import myDialog from '../../components/basic-info/tray-dialog'
export default {
    components:{
        myTable,myDialog
    },
    ...
    methods:{
        ...
        search(){
            this.$http.post('api/selectPallet',{keywords:this.keywords}).then((res)=>{
                if(res.data.code===1){
    ...
    created(){
        this.$http.get('api/getPallet').then((res)=>{
            if(res.data.code===1){
                this.getData=res.data.data;
                this.total1=this.getData.length
            }else{
