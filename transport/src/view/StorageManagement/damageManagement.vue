<template>
    <el-row class="storagebuy">
        ...
        <el-col class="search-box" :span="24">
            ...
            <div class="demo-search">报损单号：
                <el-input v-model="keywords" placeholder="请输入报损单号" size="small"></el-input>
            </div>
        ...
        <el-col :span="24" class="btn-form">
            <el-col :span="4" class="print">
                <el-button type="primary"  @click="newdocument" plain>新建报损单</el-button>
...
<script>
import myTable from '../../components/storageOperation/damageManagement-table'
import myDialog from '../../components/storageOperation/damageManagement-dialog'
export default {
    components:{
        myTable,myDialog
    },
    ...
    methods:{
        ...
        search(){
            this.$http.post('api/selectDamage',{keywords:this.keywords,value:this.value}).then((res)=>{
                if(res.data.code===1){
    ...
    created(){
        this.$http.get('api/getDamage').then((res)=>{
            if(res.data.code===1){
                this.getData=res.data.data;
                this.total1=this.getData.length
            }else{
