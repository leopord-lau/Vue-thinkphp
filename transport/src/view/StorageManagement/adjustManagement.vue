<template>
    <el-row class="storagebuy">
        ...
        <el-col class="search-box" :span="24">
            ...
            <div class="demo-search">单据号：
                <el-input v-model="keywords" placeholder="请输入入库单号" size="small"></el-input>
            </div>
        ...
        <el-col :span="24" class="btn-form">
            <el-col :span="4" class="print">
                <el-button type="primary"  @click="newdocument" plain>新建调拨单</el-button>
            </el-col>
            <el-col :span="24">
...
<script>
import myTable from '../../components/storageOperation/adjustManagement-table'
import myDialog from '../../components/storageOperation/adjustManagement-dialog'
export default {
    components:{
        myTable,myDialog
    },
    ...
    methods:{
        ...
        search(){
            this.$http.post('api/selectAdjust',{keywords:this.keywords,value:this.value}).then((res)=>{
                if(res.data.code===1){
    ...
    created(){
        this.$http.get('api/getAdjust').then((res)=>{
            if(res.data.code===1){
                this.getData=res.data.data;
                this.total1=this.getData.length
            }else{
