    ...
    <el-row class="storagebuy">
        ...
        <el-col class="search-box" :span="24">
            ...
            <div class="demo-search status">状态：
                <el-select v-model="value" placeholder="状态">
                    ...
                    <el-option value="checkpass">审核中</el-option>
                    <el-option value="done">已完成</el-option>
                </el-select> 
            </div>
        ...
        <el-col :span="24" class="btn-form">
            <el-col :span="4" class="select-btns-left">
                <el-button type="primary"  @click="newdocument" plain>新建出库单</el-button>
            </el-col>
            <el-col :span="24">
...
<script>
import myTable from '../../components/storageOperation/outStorageManagement-table'
import myDialog from '../../components/storageOperation/outStorageManagement-dialog'
export default {
    components:{
        myTable,myDialog
    },
    ...
    methods:{
        ...
        search(){
            this.$http.post('api/selectOutstore',{keywords:this.keywords,value:this.value}).then((res)=>{
                if(res.data.code===1){
    ...
    created(){
        this.$http.get('api/getOutstore').then((res)=>{
            if(res.data.code===1){
                this.getData=res.data.data;
                this.total1=this.getData.length
            }else{
